...
def load_fin_neg_list(file_path):
    import pandas as pd
    print('\nMaster Dictionary loaded from file: \n  ' + file_path)
    df = pd.read_csv(file_path)
    filtered_df = df[df['Negative'] != 0]
    print('  {0:,} fin negative words loaded in master_dictionary.'.format(len(filtered_df)) + '\n')

    # return negative word list
    return filtered_df['Word'].tolist()
