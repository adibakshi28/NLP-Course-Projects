...
PARM_PATH = r'D:\\python_working_fold\\7871NLP\\hw1\\data\\'
...
PARM_LOGFILE = (r'D:\\python_working_fold\\7871NLP\\hw1\\result\\' +
                r'EDGAR_Download_FORM-X_LogFile_' +
                str(PARM_BGNYEAR) + '-' + str(PARM_ENDYEAR) + '.txt')
# EDGAR parameter
PARM_EDGARPREFIX = 'https://www.sec.gov/Archives/'

ciks = pd.read_csv("cik.csv", parse_dates=['FTDate', 'LTDate'])
SP500_ciks = ciks['CIK'].values
def SP500filter(all_index):
    filtered=[]
    for idx in all_index:
        CIK = idx.cik
        if idx.cik in SP500_ciks and (ciks.loc[ciks['CIK'] == CIK, "FTDate"].values[0] < pd.to_datetime(str(idx.filingdate)) < ciks.loc[ciks['CIK'] == CIK, "LTDate"].values[0]):
            filtered.append(idx)
    return filtered
