...
def download_masterindex(year, qtr, flag=False):
    ...
    #############################################################################
    headers = {
        "User-Agent": "NYU - Contact: dl5182@nyu.edu"
    }
    #############################################################################
...
#             break
        ...
        try:
            ...
            request = Request(sec_url, headers=headers)
            
            # Open the URL using urlopen with the request object
            with urlopen(request) as response:
                # Read and process the zip file content
                zipfile = ZipFile(BytesIO(response.read()))
                # Read the 'master.idx' file from the zip archive, ignoring first 10 lines (header)
                records = zipfile.open('master.idx').read().decode('utf-8', 'ignore').splitlines()[10:]
